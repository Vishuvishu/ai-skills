...
2. **Auto-ban**: Any IP that hits a forbidden path is almost certainly an automated scanner, not a real user who made a typo. Blacklist that IP for the remainder of the session (in-memory, Redis, or at the edge/WAF) and reject all further requests from it without processing.
